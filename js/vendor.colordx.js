...
var h = Array.from({ length: 256 }, (_t2, r) => r.toString(16).padStart(2, '0'));
...
var p2 = (t2) => {
    ...
    const r = t2.trim();
    if (!i2.test(r)) return null;
    const o2 = r.length;
    if (o2 === 4 || o2 === 5) {
        const e = s2(r.charCodeAt(1)),
            c2 = s2(r.charCodeAt(2)),
            b$1 = s2(r.charCodeAt(3)),
            l = o2 === 5 ? s2(r.charCodeAt(4)) : 15;
...
var s3 = (r) => {
    const n2 = Math.abs(r),
        e = n2 <= 0.04045 ? n2 / 12.92 : ((n2 + 0.055) / 1.055) ** 2.4;
    return r < 0 ? -e : e;
};
var a3 = (r) => {
    const n2 = Math.abs(r),
        e = n2 <= 0.0031308 ? 12.92 * n2 : 1.055 * n2 ** (1 / 2.4) - 0.055;
    return r < 0 ? -e : e;
};
...
var b = (r) => {
    const n2 = Math.abs(r),
        e = n2 < o2 * 4.5 ? n2 / 4.5 : ((n2 + t2 - 1) / t2) ** (1 / 0.45);
    return r < 0 ? -e : e;
};
...
var x3 = (s4) => {
    if (!p(s4)) return null;
    const n2 = s4.colorSpace;
    if (n2 === 'display-p3' || n2 === 'rec2020' || !a(s4, ['r', 'g', 'b'])) return null;
    const { r: e, g: t3, b: c2, alpha: l = 1 } = s4;
    return !i(e) || !i(t3) || !i(c2) || !i(l) ? null : $({ r: c(e), g: c(t3), b: c(c2), alpha: c(l) });
};
var y = new RegExp(
    `^rgba?\\(\\s*(?<r>${N})(?<rp>%?)\\s*(?:,\\s*(?<g_c>${N})(?<gp_c>%?)\\s*,\\s*(?<b_c>${N})(?<bp_c>%?)(?:\\s*,\\s*(?<al_c>${N})(?<alp_c>%?))?\\s*|\\s+(?<g_s>${N})(?<gp_s>%?)\\s+(?<b_s>${N})(?<bp_s>%?)(?:\\s*/\\s*(?<al_s>${N})(?<alp_s>%?))?\\s*)\\)$`,
    'i'
);
var N2 = (s4) => {
    if (typeof s4 !== 'string') return null;
    const n2 = y.exec(s4.trim())?.groups;
    if (!n2) return null;
    const e = n2.g_c !== undefined,
        t3 = n2.g_c ?? n2.g_s,
        c2 = n2.b_c ?? n2.b_s,
        l$1 = !!n2.rp,
        _ = !!(n2.gp_c ?? n2.gp_s),
        b2 = !!(n2.bp_c ?? n2.bp_s),
        m2 = l$1 ? (x(n2.r) / 100) * 255 : x(n2.r),
        w = _ ? (x(t3) / 100) * 255 : x(t3),
        d = b2 ? (x(c2) / 100) * 255 : x(c2);
    if (e && (l$1 !== _ || _ !== b2 || /^none$/i.test(n2.r) || /^none$/i.test(t3) || /^none$/i.test(c2))) return null;
    const g = n2.al_c ?? n2.al_s,
        h2 = !!(n2.alp_c ?? n2.alp_s);
    if (e && g !== undefined && /^none$/i.test(g)) return null;
    const k = g === undefined ? 1 : x(g) / (h2 ? 100 : 1);
...
var $2 = 1.7076147009;
var cn = (o3, n2, r) => {
    const t3 = Math.cbrt(U * o3 + z * n2 + q * r),
        s4 = Math.cbrt(D * o3 + H * n2 + J * r),
        c2 = Math.cbrt(P * o3 + Q * n2 + V * r);
    return [W * t3 + X * s4 + Y * c2, Z * t3 + nn * s4 + on * c2, rn * t3 + tn * s4 + sn * c2];
};
var Mn = ({ r: o3, g: n2, b: r, alpha: t3 }) => {
    const [s4, c2, e] = cn(s3(o3 / 255), s3(n2 / 255), s3(r / 255));
    return { l: s4, a: c2, b: e, alpha: t3 };
};
var j = ({ l: o3, a: n2, b: r, alpha: t3 }) => {
    const [s4, c2, e] = en(o3, n2, r);
...
var un = ({ l: o3, a: n2, b: r, alpha: t3 }) => {
    ...
    const s4 = o3 + 0.3963377774 * n2 + 0.2158037573 * r,
        c2 = o3 - 0.1055613458 * n2 - 0.0638541728 * r,
        e = o3 - 0.0894841775 * n2 - 1.291485548 * r,
        _ = s4 ** 3,
        l = c2 ** 3,
        b$1 = e ** 3,
        p3 = 4.0767416613 * _ - 3.3077115904 * l + 0.2309699287 * b$1,
        E = -1.2684380041 * _ + 2.6097574007 * l - 0.3413193963 * b$1,
        F = -0.0041960865 * _ - 0.7034186145 * l + 1.7076147009 * b$1;
...
var pn = (o3) => {
    if (
        ...
        return null;
    const { l: n2, a: r, b: t3, alpha: s4 = 1 } = o3;
    return !i(n2) || !i(r) || !i(t3) || !i(s4) || c(n2) > 1
        ? null
        : j({ l: c(n2), a: c(r), b: c(t3), alpha: t(c(s4), 0, 1) });
};
var _n = new RegExp(
    `^oklab\\(\\s*(?<l>${N})(?<lp>%?)\\s+(?<a>${N})(?<ap>%?)\\s+(?<b>${N})(?<bp>%?)\\s*(?:/\\s*(?<al>${N})(?<alp>%?)\\s*)?\\)$`,
    'i'
);
var vn = (o3) => {
    if (typeof o3 !== 'string') return null;
    const n2 = _n.exec(o3.trim())?.groups;
    if (!n2) return null;
    const r = n2.lp ? x(n2.l) / 100 : x(n2.l),
        t3 = n2.ap ? x(n2.a) * 0.004 : x(n2.a),
        s4 = n2.bp ? x(n2.b) * 0.004 : x(n2.b),
        c2 = n2.al === undefined ? 1 : x(n2.al) / (n2.alp ? 100 : 1);
...
var M = (n2, o3, t3) => {
    const s4 = xo * n2 + go * o3 + mo * t3,
        r = Do * n2 + zo * o3 + io * t3,
        l = Xo * n2 + ho * o3 + Co * t3;
    return [A2 * s4 + B2 * r + G2 * l, I2 * s4 + P2 * r + U2 * l, F * s4 + H2 * r + K * l];
};
var _Fo = new RegExp(
    `^color\\(\\s*xyz-d65\\s+(?<x>${N})(?<xp>%?)\\s+(?<y>${N})(?<yp>%?)\\s+(?<z>${N})(?<zp>%?)\\s*(?:/\\s*(?<al>${N})(?<alp>%?)\\s*)?\\)$`,
    'i'
);
var _Ho = new RegExp(
    `^color\\(\\s*xyz-d50\\s+(?<x>${N})(?<xp>%?)\\s+(?<y>${N})(?<yp>%?)\\s+(?<z>${N})(?<zp>%?)\\s*(?:/\\s*(?<al>${N})(?<alp>%?)\\s*)?\\)$`,
    'i'
);
var _Ko = (0.3127 / 0.329) * 100;
var _Vo = ((1 - 0.3127 - 0.329) / 0.329) * 100;
var S2 = 216 / 24389;
var z2 = 24389 / 27;
var Lo = (n2, o3, t3) => {
    const s4 = (n2 + 16) / 116,
        ...
        l = s4 - t3 / 200;
    return [
        (r ** 3 > S2 ? r ** 3 : (116 * r - 16) / z2) * L,
        (n2 > 8 ? s4 ** 3 : n2 / z2) * O2,
        (l ** 3 > S2 ? l ** 3 : (116 * l - 16) / z2) * w2
    ];
};
var Qo = ({ l: n2, a: o3, b: t3, alpha: s4 }) => {
    const [r, l, a4] = Lo(n2, o3, t3);
    return { x: r, y: l, z: a4, alpha: s4 };
};
var _on2 = new RegExp(
    `^lab\\(\\s*(?<l>${N})(?<lp>%?)\\s+(?<a>${N})(?<ap>%?)\\s+(?<b>${N})(?<bp>%?)\\s*(?:/\\s*(?<al>${N})(?<alp>%?)\\s*)?\\)$`,
    'i'
);
...
var _jr = new RegExp(
    `^color\\(\\s*display-p3\\s+(?<r>${N})(?<rp>%?)\\s+(?<g>${N})(?<gp>%?)\\s+(?<b>${N})(?<bp>%?)\\s*(?:/\\s*(?<al>${N})(?<alp>%?)\\s*)?\\)$`,
    'i'
);
...
var _Qr = new RegExp(
    `^color\\(\\s*rec2020\\s+(?<r>${N})(?<rp>%?)\\s+(?<g>${N})(?<gp>%?)\\s+(?<b>${N})(?<bp>%?)\\s*(?:/\\s*(?<al>${N})(?<alp>%?)\\s*)?\\)$`,
    'i'
);
var Wr =
...
var A3 = (e, r, n2) => {
    const { x: o3, y: b3, z: t3 } = Qo({ l: e, a: r, b: n2, alpha: 1, colorSpace: 'lab' }),
        [u2, s4, l] = M(o3, b3, t3);
    return cn(u2, s4, l);
};
var Tr = (e, r, n2) => {
    const [o3, b3, t3] = z3(s3(e), s3(r), s3(n2));
    return cn(o3, b3, t3);
};
var Cr = (e, r, n2) => {
    const [o3, b3, t3] = F2(b(e), b(r), b(n2));
    return cn(o3, b3, t3);
};
var Lr = (e, r, n2) => {
    const [o3, b3, t3] = M(e, r, n2);
    return cn(o3, b3, t3);
};
var Nr = (e, r, n2) => {
    const [o3, b3, t3] = Uo(e, r, n2);
    return cn(o3, b3, t3);
};
var G3 = (e) => {
    if (typeof e === 'object' && e !== null) {
        ...
        if ('l' in r && 'c' in r && 'h' in r && r.colorSpace !== 'lch' && !('a' in r) && !('r' in r)) {
            ...
            if (typeof n2.l === 'number' && typeof n2.c === 'number' && typeof n2.h === 'number') {
                const o3 = (n2.h * Math.PI) / 180;
                return { l: n2.l, a: n2.c * Math.cos(o3), b: n2.c * Math.sin(o3), alpha: n2.alpha };
            }
        }
        if (r.colorSpace === 'lab') {
            const n2 = e;
            if (typeof n2.l !== 'number' || typeof n2.a !== 'number' || typeof n2.b !== 'number') return null;
            const [o3, b3, t3] = A3(n2.l, n2.a, n2.b);
            return { l: o3, a: b3, b: t3, alpha: typeof n2.alpha === 'number' ? n2.alpha : 1 };
        }
        if (r.colorSpace === 'lch') {
            const n2 = e;
            if (typeof n2.l !== 'number' || typeof n2.c !== 'number' || typeof n2.h !== 'number') return null;
            const o3 = (n2.h * Math.PI) / 180,
                [b3, t3, u2] = A3(n2.l, n2.c * Math.cos(o3), n2.c * Math.sin(o3));
            return { l: b3, a: t3, b: u2, alpha: typeof n2.alpha === 'number' ? n2.alpha : 1 };
        }
        if (r.colorSpace === 'display-p3') {
            const n2 = e;
            if (typeof n2.r !== 'number' || typeof n2.g !== 'number' || typeof n2.b !== 'number') return null;
            const [o3, b3, t3] = Tr(n2.r, n2.g, n2.b);
            return { l: o3, a: b3, b: t3, alpha: typeof n2.alpha === 'number' ? n2.alpha : 1 };
        }
        if (r.colorSpace === 'rec2020') {
            const n2 = e;
            if (typeof n2.r !== 'number' || typeof n2.g !== 'number' || typeof n2.b !== 'number') return null;
            const [o3, b3, t3] = Cr(n2.r, n2.g, n2.b);
            return { l: o3, a: b3, b: t3, alpha: typeof n2.alpha === 'number' ? n2.alpha : 1 };
        }
        if (r.colorSpace === 'xyz-d65') {
            const n2 = e;
            if (typeof n2.x !== 'number' || typeof n2.y !== 'number' || typeof n2.z !== 'number') return null;
            const [o3, b3, t3] = Nr(n2.x, n2.y, n2.z);
            return { l: o3, a: b3, b: t3, alpha: typeof n2.alpha === 'number' ? n2.alpha : 1 };
        }
        if ('x' in r && 'y' in r && 'z' in r) {
            const n2 = e;
            if (typeof n2.x !== 'number' || typeof n2.y !== 'number' || typeof n2.z !== 'number') return null;
            const [o3, b3, t3] = Lr(n2.x, n2.y, n2.z);
            return { l: o3, a: b3, b: t3, alpha: typeof n2.alpha === 'number' ? n2.alpha : 1 };
        }
        return null;
    }
    if (typeof e === 'string') {
        let r = Wr.exec(e);
        if (r) {
            const n2 = r[2] ? Number(r[1]) / 100 : Number(r[1]),
                o3 = r[4] ? Number(r[3]) * 0.004 : Number(r[3]),
                b3 = r[6]?.toLowerCase() ?? 'deg',
                u2 = (Number(r[5]) * (m[b3] ?? 1) * Math.PI) / 180,
                s4 = r[7] === undefined ? 1 : Number(r[7]) / (r[8] ? 100 : 1);
            return { l: n2, a: o3 * Math.cos(u2), b: o3 * Math.sin(u2), alpha: s4 };
        }
        if (((r = rn2.exec(e)), r)) {
            const n2 = r[2] ? Number(r[1]) / 100 : Number(r[1]),
                o3 = r[4] ? Number(r[3]) * 0.004 : Number(r[3]),
                b3 = r[6] ? Number(r[5]) * 0.004 : Number(r[5]),
                t3 = r[7] === undefined ? 1 : Number(r[7]) / (r[8] ? 100 : 1);
            return { l: n2, a: o3, b: b3, alpha: t3 };
        }
        if (((r = nn2.exec(e)), r)) {
            const n2 = Number(r[1]),
                o3 = r[4] ? Number(r[3]) * 1.25 : Number(r[3]),
                b3 = r[6] ? Number(r[5]) * 1.25 : Number(r[5]),
                t3 = r[7] === undefined ? 1 : Number(r[7]) / (r[8] ? 100 : 1),
                [u2, s4, l] = A3(n2, o3, b3);
        ...
        if (((r = en2.exec(e)), r)) {
            const n2 = Number(r[1]),
                o3 = r[4] ? Number(r[3]) * 1.5 : Number(r[3]),
                b3 = r[6]?.toLowerCase() ?? 'deg',
                u2 = (Number(r[5]) * (m[b3] ?? 1) * Math.PI) / 180,
                s4 = r[7] === undefined ? 1 : Number(r[7]) / (r[8] ? 100 : 1),
                [l, x5, T2] = A3(n2, o3 * Math.cos(u2), o3 * Math.sin(u2));
            return { l, a: x5, b: T2, alpha: s4 };
        }
        if (((r = on3.exec(e)), r)) {
            const n2 = r[2] ? Number(r[1]) / 100 : Number(r[1]),
                o3 = r[4] ? Number(r[3]) / 100 : Number(r[3]),
                b3 = r[6] ? Number(r[5]) / 100 : Number(r[5]),
                t3 = r[7] === undefined ? 1 : Number(r[7]) / (r[8] ? 100 : 1),
                [u2, s4, l] = Tr(n2, o3, b3);
            return { l: u2, a: s4, b: l, alpha: t3 };
        }
        ...
        if (((r = tn2.exec(e)), r)) {
            const n2 = Number(r[1]),
                o3 = Number(r[2]),
                b3 = Number(r[3]),
                t3 = r[4] === undefined ? 1 : Number(r[4]) / (r[5] ? 100 : 1),
                [u2, s4, l] = Lr(n2, o3, b3);
            return { l: u2, a: s4, b: l, alpha: t3 };
        }
        if (((r = an.exec(e)), r)) {
...
var Ar = (e, r, n2, o3, b3) => {
    if (e >= 1) return [1, 1, 1];
    if (e <= 0) return [0, 0, 0];
    const [t3, u2, s4] = o3(e, r, n2);
    if (_r(t3, u2, s4)) return [t3, u2, s4];
    const l = t(t3, 0, 1),
        x5 = t(u2, 0, 1),
        T2 = t(s4, 0, 1);
    if (Mr(b3(l, x5, T2), [e, r, n2]) <= kr) return [l, x5, T2];
    let E = Math.atan2(n2, r),
        wr = Math.sqrt(r * r + n2 * n2),
        N4 = 0,
        I3 = wr,
        $3 = true,
        D2 = l,
...
var O3 = ({ r: n2, g: r, b: o3, alpha: t3 }) => {
    ...
    const k = d2 * 360;
    return (
        (y3.h = k >= 0 && k < 360 ? k : ((k % 360) + 360) % 360),
        (y3.s = t(f3 * 100, 0, 100)),
        (y3.l = t(m2 * 100, 0, 100)),
        (y3.alpha = t(s(t3, 3), 0, 1)),
        y3
    );
};
var v = (n2, r, o3) => (
    o3 < 0 && (o3 += 1),
    o3 > 1 && (o3 -= 1),
    o3 < 1 / 6 ? n2 + (r - n2) * 6 * o3 : o3 < 1 / 2 ? r : o3 < 2 / 3 ? n2 + (r - n2) * (2 / 3 - o3) * 6 : n2
);
var _ = ({ h: n2, s: r, l: o3, alpha: t3 }) => {
    const e = r / 100,
        l = o3 / 100,
        s4 = l < 0.5 ? l * (1 + e) : l + e - l * e,
        a4 = 2 * l - s4,
        c3 = (((n2 % 360) + 360) % 360) / 360;
    return { r: v(a4, s4, c3 + 1 / 3) * 255, g: v(a4, s4, c3) * 255, b: v(a4, s4, c3 - 1 / 3) * 255, alpha: t3 };
};
var V2 = (n2) => {
    if (!p(n2) || !a(n2, ['h', 's', 'l'])) return null;
    const { h: r, s: o3, l: t3, alpha: e = 1 } = n2;
    return !i(r) || !i(o3) || !i(t3) || !i(e) ? null : _(X2({ h: c(r), s: c(o3), l: c(t3), alpha: c(e) }));
};
var Lr2 = new RegExp(
    `^hsla?\\(\\s*(?<h>${N})(?<hu>deg|rad|grad|turn)?\\s*(?:,\\s*(?<s_c>${o})%\\s*,\\s*(?<l_c>${o})%(?:\\s*,\\s*(?<al_c>${o})(?<alp_c>%?)?\\s*)?|\\s+(?<s_s>${N})(?<sp_s>%?)\\s+(?<l_s>${N})(?<lp_s>%?)(?:\\s*/\\s*(?<al_s>${N})(?<alp_s>%?)?\\s*)?)\\)$`,
    'i'
);
var J2 = (n2) => {
    if (typeof n2 !== 'string') return null;
    const r = Lr2.exec(n2.trim())?.groups;
    if (!r) return null;
    const o3 = r.s_c !== undefined;
    if (o3 && /^none$/i.test(r.h)) return null;
    const t3 = r.hu?.toLowerCase() ?? 'deg',
        e = x(r.h) * (m[t3] ?? 1),
        l$1 = x(r.s_c ?? r.s_s),
        s4 = x(r.l_c ?? r.l_s),
        a4 = r.al_c ?? r.al_s,
        c3 = !!(r.alp_c ?? r.alp_s);
    if (o3 && a4 !== undefined && /^none$/i.test(a4)) return null;
    const m2 = a4 === undefined ? 1 : x(a4) / (c3 ? 100 : 1);
    return _(X2({ h: e, s: l$1, l: s4, alpha: m2 }));
};
var Q2 = ({ l: n2, c: r, h: o3, alpha: t3 }) => ({
    l: n2,
    a: r * Math.cos((o3 * Math.PI) / 180),
    b: r * Math.sin((o3 * Math.PI) / 180),
    alpha: t3
});
var W2 = (n2) => {
    const { l: r, a: o3, b: t3, alpha: e } = Mn(n2),
        l = Math.sqrt(o3 * o3 + t3 * t3),
        s4 = (Math.atan2(t3, o3) * 180) / Math.PI;
    return { l: r, c: l, h: l < 0.000004 ? 0 : u(s4), alpha: e };
};
var M2 = (n2) => un(Q2(n2));
var Y2 = (n2) => j(Q2(n2));
var Z2 = (n2) => {
    if (!p(n2) || n2.colorSpace === 'lch' || !a(n2, ['l', 'c', 'h'])) return null;
    const { l: r, c: o3, h: t3, alpha: e = 1 } = n2;
    return !i(r) || !i(o3) || !i(t3) || !i(e) || c(r) > 1
        ? null
        : Y2({ l: c(r), c: Math.max(0, c(o3)), h: u(c(t3)), alpha: t(c(e), 0, 1) });
};
var Ir = new RegExp(
    `^oklch\\(\\s*(?<l>${N})(?<lp>%?)\\s+(?<c>${N})(?<cp>%?)\\s+(?<h>${N})(?<hu>deg|rad|grad|turn)?\\s*(?:/\\s*(?<al>${N})(?<alp>%?)\\s*)?\\)$`,
    'i'
);
var rr = (n2) => {
    if (typeof n2 !== 'string') return null;
    const r = Ir.exec(n2.trim())?.groups;
    if (!r) return null;
    const o3 = r.lp ? x(r.l) / 100 : x(r.l),
        t3 = Math.max(0, r.cp ? x(r.c) * 0.004 : x(r.c)),
        e = r.hu?.toLowerCase() ?? 'deg',
        l$1 = x(r.h) * (m[e] ?? 1),
        s4 = r.al === undefined ? 1 : x(r.al) / (r.alp ? 100 : 1);
...
var N4 = [];
var sr = (n2) => {
    if (n2 === 'transparent') return { r: 0, g: 0, b: 0, alpha: 0 };
    const r = typeof n2 === 'string' ? nr : lr;
    for (const o3 of r) {
        const t3 = o3(n2);
        if (t3) return t3;
    }
...
var u2 = class n2 {
    ...
    _valid;
    constructor(r, o3) {
        if (r === ar) (this._valid = true), (this._rgb = o3);
    ...
    static _makeFromLinearSrgb(r, o3, t3, e) {
        const l = a3(r) * 255,
            ...
            a4 = a3(t3) * 255;
        return n2._make({
            r: l >= 0 && l < 0.5 ? 0 : l > 254.5 && l <= 255 ? 255 : l,
            g: s4 >= 0 && s4 < 0.5 ? 0 : s4 > 254.5 && s4 <= 255 ? 255 : s4,
            b: a4 >= 0 && a4 < 0.5 ? 0 : a4 > 254.5 && a4 <= 255 ? 255 : a4,
            alpha: e
        });
    }
    isValid() {
    ...
    toRgb() {
        const { r, g: o3, b: t3, alpha: e } = this._rgb;
    ...
    toRgbString(r) {
        const { r: o3, g: t3, b: e, alpha: l } = this._rgb,
            s4 = t(s(o3), 0, 255),
            a$1 = t(s(t3), 0, 255),
            c3 = t(s(e), 0, 255);
        return r?.legacy
    ...
    toNumber() {
        const { r, g: o3, b: t3 } = this._rgb;
        return (t(s(r), 0, 255) << 16) | (t(s(o3), 0, 255) << 8) | t(s(t3), 0, 255);
    }
    toHsl(r = 2) {
        const { h: o3, s: t3, l: e, alpha: l } = O3(this._rgb),
            s4 = s(o3, r);
        return { h: s4 >= 360 ? 0 : s4, s: s(t3, r), l: s(e, r), alpha: l };
    }
    toHslString(r = 2) {
        const { h: o3, s: t3, l: e, alpha: l } = this.toHsl(r);
        return l < 1 ? `hsl(${o3} ${t3}% ${e}% / ${l})` : `hsl(${o3} ${t3}% ${e}%)`;
    }
    toOklab(r = 5) {
        const { l: o3, a: t3, b: e, alpha: l } = Mn(this._rgb);
        return { l: s(o3, r), a: s(t3, r), b: s(e, r), alpha: l };
    }
    toOklabString(r = 5) {
        const { l: o3, a: t3, b: e, alpha: l } = this.toOklab(r);
        return l < 1 ? `oklab(${o3} ${t3} ${e} / ${l})` : `oklab(${o3} ${t3} ${e})`;
    }
    toOklch(r = 5) {
        const { l: o3, c: t3, h: e, alpha: l } = W2(this._rgb);
        return { l: s(o3, r), c: s(t3, r), h: s(e, r), alpha: l };
    }
    toOklchString(r = 5) {
        const { l: o3, c: t3, h: e, alpha: l } = this.toOklch(r),
            s4 = t3 === 0 ? 'none' : e;
    ...
    lighten(r = 0.1, o3) {
        const { h: t3, s: e, l, alpha: s4 } = O3(this._rgb),
            a$1 = o3?.relative ? l * (1 + r) : l + r * 100;
    ...
    clampSrgb() {
        const { r, g: o3, b: t3, alpha: e } = this._rgb;
        return r >= 0 && r <= 255 && o3 >= 0 && o3 <= 255 && t3 >= 0 && t3 <= 255
            ? this
            : n2._make({ r: t(r, 0, 255), g: t(o3, 0, 255), b: t(t3, 0, 255), alpha: e });
    }
    mapSrgb() {
        const { r, g: o3, b: t3, alpha: e } = this._rgb;
        if (r >= 0 && r <= 255 && o3 >= 0 && o3 <= 255 && t3 >= 0 && t3 <= 255) return this;
        const [l, s4, a4] = cn(s3(r / 255), s3(o3 / 255), s3(t3 / 255)),
            c3 = l > 1 - 0.0000001 ? 1 : l < 0.0000001 ? 0 : l,
            m2 = Dn({ l: c3, a: s4, b: a4, alpha: e });
        if (m2 === null) return this;
        const [d2, f3, k] = m2.linear;
...
u2.toGamutSrgb = (n3) => {
    const r = Dn(n3);
    if (r === null) return new u2(n3);
    const [o3, t3, e] = r.linear;
...
function A4(e, t3, n3) {
    const o3 = (l) => (l / 255) ** 2.4,
        r = f3.Rsco * o3(e) + f3.Gsco * o3(t3) + f3.Bsco * o3(n3);
    return r > a4.blkThrs ? r : r + (a4.blkThrs - r) ** a4.blkClmp;
}
function p3(e, t3) {
    const n3 = A4(e.r, e.g, e.b),
        o3 = A4(t3.r, t3.g, t3.b);
    if (Math.abs(o3 - n3) < a4.deltaYmin) return 0;
    if (o3 > n3) {
        const r = (o3 ** a4.normBG - n3 ** a4.normTXT) * a4.scale;
        return r < a4.loClip ? 0 : (r - a4.offset) * 100;
    } else {
        const r = (o3 ** a4.revBG - n3 ** a4.revTXT) * a4.scale;
        return r > -0.1 ? 0 : (r + a4.offset) * 100;
    }
}
var h3 = (e) => {
    (e.prototype.luminance = function () {
        const { r: t3, g: n3, b: o3 } = this._rawRgb();
        ...
        (e.prototype.contrast = function (t3 = '#fff') {
            const n3 = new e(t3),
                ...
                s4 = (
                    r.alpha < 1
                        ? new e({
                              r: r.alpha * r.r + (1 - r.alpha) * o3.r,
                              g: r.alpha * r.g + (1 - r.alpha) * o3.g,
                              b: r.alpha * r.b + (1 - r.alpha) * o3.b,
                              alpha: 1
                          })
                        : this
                ).luminance(),
        ...
        (e.prototype.apcaContrast = function (t3 = '#fff') {
            const n3 = new e(t3).toRgb(),
                o3 = this.toRgb(),
                r =
                    o3.alpha < 1
                        ? {
                              r: o3.alpha * o3.r + (1 - o3.alpha) * n3.r,
                              g: o3.alpha * o3.g + (1 - o3.alpha) * n3.g,
                              b: o3.alpha * o3.b + (1 - o3.alpha) * n3.b
                          }
                        : o3;
            return Math.round(p3(r, n3) * 10) / 10;
        }),
        (e.prototype.isReadableApca = function (t3 = '#fff', n3 = {}) {
            const { size: o3 = 'normal' } = n3,
                r = Math.abs(this.apcaContrast(t3));
            return o3 === 'large' ? r >= 60 : r >= 75;
        }),
        (e.prototype.readableScore = function (t3 = '#fff') {
            const n3 = this.contrast(t3);
            return n3 >= 7 ? 'AAA' : n3 >= 4.5 ? 'AA' : n3 >= 3 ? 'AA large' : 'fail';
        }),
        (e.prototype.isReadable = function (t3 = '#fff', n3 = {}) {
            const { level: o3 = 'AA', size: r = 'normal' } = n3,
                l = this.contrast(t3);
            return o3 === 'AAA' ? (r === 'large' ? l >= 4.5 : l >= 7) : r === 'large' ? l >= 3 : l >= 4.5;
        }),
        (e.prototype.minReadable = function (t3 = '#fff') {
            const n3 = new e(t3).luminance(),
                o3 = (n3 + 0.05) / 0.05,
                r = 1.05 / (n3 + 0.05),
                l = o3 >= r,
                s4 = (i3) => (l ? i3.darken(0.01) : i3.lighten(0.01));
            if (this.contrast(t3) >= 4.5) return this;
            let c3 = s4(this);
            for (let i3 = 1; i3 < 100 && !(c3.contrast(t3) >= 4.5); i3++) c3 = s4(c3);
            return c3;
        });
};
var y4 = h3;
